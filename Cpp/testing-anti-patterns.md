...
TEST(PageTest, RendersSidebar) {
    ...
    page.render();
    // EXPECT_CALL verification is on the mock, not on Page's actual output
}
```
...
TEST(PageTest, RendersSidebar) {
    Page page(std::make_unique<RealSidebar>()); // Don't mock sidebar
    std::string output = page.render();
    EXPECT_THAT(output, HasSubstr("<nav"));
}

// OR if Sidebar must be mocked for isolation:
// Don't set expectations on the mock - test Page's behavior with sidebar present
```
...
// In test_utils/session_test_utils.h
...
TEST(ServerTest, DetectsDuplicateServer) {
    ...
    MockMCPServerManager mock_manager; // Just mock slow server startup
...
// Later: crashes when code accesses mock_response.metadata.request_id
```

**Why this is wrong:**
- **Partial mocks hide structural assumptions** - You only mocked fields you know about
- **Downstream code may depend on fields you didn't include** - Silent failures or crashes
...
| Anti-Pattern | Fix |
|--------------|-----|
| Assert on mock interactions instead of output | Test real component or unmock it |
...
- Assertions only verify mock interactions, not real output
