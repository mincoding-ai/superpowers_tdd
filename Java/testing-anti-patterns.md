...
void renders_sidebar() {
    ...
    page.render();
    verify(mockSidebar).render(); // Testing mock was invoked, not that Page works
}
```
...
@Test
void renders_sidebar() {
    Page page = new Page(new RealSidebar()); // Don't mock sidebar
...
void detects_duplicate_server() throws Exception {
    ...
    MCPServerManager mockManager = mock(MCPServerManager.class); // Just mock slow server startup
