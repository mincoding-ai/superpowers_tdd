...
def test_renders_sidebar(mocker):
    ...
    page.render()
    mock_sidebar.render.assert_called_once()  # Testing mock was called, not that Page works
```
...
def test_renders_sidebar():
    page = Page(RealSidebar())  # Don't mock sidebar
    output = page.render()
...
# In test_utils/session_test_utils.py
...
def test_detects_duplicate_server(mocker):
    ...
    mocker.patch.object(MCPServerManager, 'start')  # Just mock slow server startup
...
# Later: raises AttributeError when code accesses response.metadata.request_id
```

**Why this is wrong:**
- **Partial mocks hide structural assumptions** - You only mocked fields you know about
- **Downstream code may depend on fields you didn't include** - Silent failures or AttributeError
